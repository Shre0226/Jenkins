  ...
  stages {
    ...
    stage('Deploy image in production EC2') {
      ...
      steps{
        ...
        sh 'ssh -o StrictHostKeyChecking=no ec2-user@54.197.9.239 aws ecr get-login --no-include-email --region us-west-2 | docker login --username AWS --password-stdin 853973692277.dkr.ecr.us-east-1.amazonaws.com'
